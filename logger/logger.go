...
func init() {
	Debug = newLogger(
		FlagDebug,
		log.New(os.Stderr, "SALIENTWM DEBUG: *** ", logFlags),
		log.New(os.Stderr,
			BgGreen(Blue("SALIENTWM DEBUG:")).String()+" ", logFlags))
	Lots = newLogger(
		FlagLots,
		log.New(os.Stderr, "SALIENTWM LOTS: ", logFlags),
		log.New(os.Stderr, "SALIENTWM LOTS: ", logFlags))
	Message = newLogger(
		FlagMessage,
		log.New(os.Stderr, "SALIENTWM MESSAGE: ", logFlags),
		log.New(os.Stderr, "SALIENTWM MESSAGE: ", logFlags))
	Warning = newLogger(
		FlagWarning,
		log.New(os.Stderr, "SALIENTWM WARNING: ", logFlags),
		log.New(os.Stderr,
			Bold(Red("SALIENTWM WARNING:")).String()+" ", logFlags))
	Error = newLogger(
		FlagError,
		log.New(os.Stderr, "SALIENTWM ERROR: ", logFlags),
		log.New(os.Stderr, BgMagenta("SALIENTWM ERROR:").String()+" ", logFlags))
}
